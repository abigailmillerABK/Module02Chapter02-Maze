...
{
	std::vector<PlacableActor*>::iterator listPtr;
	for (listPtr = m_pActors.begin(); listPtr < m_pActors.end(); listPtr++) {
		if ((*listPtr)->GetXPosition() == x && (*listPtr)->GetYPosition() == y) {
			m_pActors.erase(listPtr);
			return;
		}
	}
}
...
bool Level::ConvertLevel(int* playerX, int* playerY)
{
	...
	for (int y = 0; y < m_height; ++y)
	{
		for (int x = 0; x < m_width; ++x)
		{
			...
			case '~':
				m_pLevelData[index] = ' ';
				m_pActors.push_back(new Box(x, y));
				break;
			case 'R':
				m_pLevelData[index] = ' ';
